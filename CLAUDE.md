...
- `lib/` — `firebase.ts` (app/auth/Firestore init + FirebaseUI), `openfoodfacts.ts` (fetch + parse helpers)
...
**Auth & data:** Firebase Auth via `@firebase-oss/ui-react` + a custom `AuthProvider` context. Firestore is initialised but not yet actively used. All Firebase config comes from `NEXT_PUBLIC_FIREBASE_*` env vars.
