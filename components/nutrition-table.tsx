...
export function NutritionTable({
  ...
  const t = useTranslations("NutritionTable");
  ...
  const displayRows = allKeysOrdered
    ...
    .map((key) => {
      const rowKey = key === COMPUTED_SCORE_KEY ? COMPUTED_SCORE_KEY : key;
      return {
        key: rowKey,
        label: tNutrients(rowKey as Parameters<typeof tNutrients>[0]),
      };
    })
  ...
  function handleShare(code?: string) {
    ...
    // Build URL preserving the locale prefix
    const localePrefix =
      window.location.pathname.match(/^\/(en|fi)/)?.[0] ?? "";
    const url = `${window.location.origin}${localePrefix}/compare?codes=${encodeURIComponent(codes)}`;
    ...
    >
      ...
      <div className="mb-3 flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {t("productCount_other", { count: products.length })}
        </p>

        <div className="flex items-center gap-1">
          <button
            onClick={toggleExpanded}
            aria-label={expanded ? t("collapseTable") : t("expandTable")}
                ...
                <>
                  <DropdownMenuSub>
                    <DropdownMenuSubTrigger className="flex max-w-full gap-1">
                      <span className="shrink-0">{t("rulesetLabel")}</span>
                ...
                <>
                  {isDirty && loadedComparisonName && onUpdateComparison && (
                    ...
                    >
                      ...
                      {t("updateComparison", { name: loadedComparisonName })}
                    </DropdownMenuItem>
                  )}
                  ...
                  {loadedComparisonName && onUnsaveComparison && (
                    ...
                    >
                      ...
                      {t("deleteComparison", { name: loadedComparisonName })}
                    </DropdownMenuItem>
                  )}
                </>
              )}
