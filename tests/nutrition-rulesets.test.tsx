...
describe("NutritionTab — rulesets list", () => {
  it("renders name, view, and delete controls for each ruleset", async () => {
    ...
    expect(
      screen.getAllByRole("button", { name: /view ruleset/i }),
    ).toHaveLength(2);
    expect(
      screen.getAllByRole("button", { name: /delete ruleset/i }),
    ).toHaveLength(2);
  });
  ...
  it("confirming deletion removes the ruleset from the list", async () => {
    ...
    fireEvent.click(
      screen.getAllByRole("button", { name: /delete ruleset/i })[0],
    );
    await waitFor(() =>
      expect(screen.getByText(/delete ruleset\?/i)).toBeInTheDocument(),
    );
    ...
    await waitFor(() =>
      expect(mockSaveNutritionSettings).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          rulesets: [
            expect.objectContaining({ id: "r1", name: "Dairy Products" }),
          ],
        }),
      ),
    );
